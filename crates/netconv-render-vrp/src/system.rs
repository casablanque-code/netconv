    ...
    for comm in &snmp.communities {
        ...
        if vrp_reserved.contains(&comm.name.to_lowercase().as_str()) {
            ...
            report.add_manual(
                ...
                &src,
                &format!(
                    "community name '{}' conflicts with VRP reserved keyword",
                    comm.name
                ),
...
fn render_stp(cfg: &NetworkConfig, out: &mut Vec<String>, report: &mut ConversionReport) {
    ...
    if matches!(stp.mode, StpMode::RapidPvst | StpMode::Pvst) {
        ...
        out.push(format!(
            "# ⚠ HIGH RISK: Cisco {} использует per-VLAN STP деревья.",
            vrp_mode
        ));
        out.push("#   На Huawei RSTP — одно дерево для всех VLAN.".to_string());
        out.push(
            "#   Если разные VLAN имели разные root bridge — топология изменится.".to_string(),
        );
        out.push("#".to_string());
    ...
    for vp in &stp.vlan_priorities {
        ...
        let vlan_str = vp
            ...
            .join(",");
        out.push(format!(
            "# MANUAL: spanning-tree vlan {} priority {} →",
            vlan_str, vp.priority
        ));
        out.push(format!(
            "#   В RSTP нет per-vlan приоритетов. Используй MSTP:"
        ));
...
fn render_voice_vlan_global(
    ...
    out.push("#".to_string());
    out.push(
        "# Voice VLAN — глобальная активация (обязательно перед интерфейсными командами)"
            .to_string(),
    );
...
fn render_users(cfg: &NetworkConfig, out: &mut Vec<String>, report: &mut ConversionReport) {
    ...
    for user in &cfg.users {
        ...
        out.push(format!(
            "local-user {} password irreversible-cipher <НОВЫЙ-ПАРОЛЬ>",
            user.name
        ));
        out.push(format!(
        ...
        out.push(format!(
            "local-user {} service-type ssh terminal",
            user.name
        ));
        ...
        report.add_manual(
            ...
            pw_note,
            Some(&format!(
                "local-user {} password irreversible-cipher <ПАРОЛЬ>",
                user.name
            )),
        );
    }
...
fn render_aaa(cfg: &NetworkConfig, out: &mut Vec<String>, report: &mut ConversionReport) {
    ...
    out.push("#".to_string());
    out.push(
        "#   For TACACS+/RADIUS — configure server-group and reference in domain.".to_string(),
    );
    report.add_manual(
        "aaa",
        "aaa new-model",
...
fn render_platform_specific(cfg: &NetworkConfig, out: &mut Vec<String>) {
    if cfg.platform_specific.is_empty() {
        return;
    }
    out.push("#".to_string());
    out.push("# ============================================================".to_string());
    ...
    for block in &cfg.platform_specific {
        ...
        } else if block.raw.starts_with("enable secret") || block.raw.starts_with("enable password")
        {
            out.push("#".to_string());
            out.push(
                "# MANUAL: Cisco enable secret/password cannot be migrated directly.".to_string(),
            );
            out.push(
                "#   On VRP there is no direct equivalent of enable-mode password.".to_string(),
            );
            ...
            out.push(
                "#       local-user admin password irreversible-cipher <PASSWORD>".to_string(),
            );
