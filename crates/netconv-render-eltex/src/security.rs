use netconv_core::ir::*;
use netconv_core::report::{ConfidenceLevel, ConversionReport, RiskLevel};
...
impl InterfaceRole {
    ...
    pub fn access_vlan(&self) -> Option<u16> {
        if let InterfaceRole::AccessEndpoint { vlan } = self {
            *vlan
        } else {
            None
        }
...
pub fn classify_zone(iface: &Interface, cfg: &NetworkConfig) -> ZoneClassification {
    ...
    if let Some(desc) = &iface.description {
        ...
        for kw in &[
            "uplink", "wan", "isp", "internet", "external", "provider", "upstream", "transit",
            "peering",
        ] {
            if d.contains(kw) {
                return ZoneClassification {
            ...
            if let Some(addr) = iface.addresses.first() {
                if let NextHop::Ip(nh) = &route.next_hop {
                    if addr.prefix.contains(nh) {
                        let is_vlan1 =
                            matches!(&iface.name.kind, InterfaceKind::Vlan) && iface.name.id == "1";
                        return ZoneClassification {
                            zone: ZoneName::Wan,
                            confidence: ConfidenceLevel::Medium,
                            reason: if is_vlan1 {
                                "default route via VLAN1 — likely management".to_string()
                            } else {
                                "default route next-hop on this interface".to_string()
                            },
                            role: if is_vlan1 {
    ...
    if let Some(l2) = &iface.l2 {
        return match l2.mode {
            L2Mode::Trunk => ZoneClassification {
            ...
            L2Mode::Access => ZoneClassification {
                zone: ZoneName::Unknown,
                confidence: ConfidenceLevel::Low,
                reason: format!(
                    "access port{}",
                ...
                role: InterfaceRole::AccessEndpoint {
                    vlan: l2.access_vlan,
                },
            },
        };
    ...
    ZoneClassification {
        zone: ZoneName::Unknown,
        confidence: ConfidenceLevel::Low,
        reason: "no classification signals".to_string(),
        role: InterfaceRole::Unknown,
    }
}

pub fn render_security_zones(
    cfg: &NetworkConfig,
    out: &mut Vec<String>,
    report: &mut ConversionReport,
) {
    let cls: Vec<(&Interface, ZoneClassification)> = cfg
        .interfaces
        .iter()
    ...
    let unknown_ct = cls
        .iter()
        .filter(|(_, z)| z.zone == ZoneName::Unknown)
        .count();
    let access_ct = cls
        .iter()
        .filter(|(_, z)| matches!(z.role, InterfaceRole::AccessEndpoint { .. }))
        .count();
    let trunk_ct = cls
        .iter()
        .filter(|(_, z)| z.role == InterfaceRole::TrunkInterswitch)
        .count();
    let med_wan_ct = cls
        .iter()
    ...
    if unknown_ct > 0 {
        out.push(format!(
            "! {} zones NOT declared — assign manually before adding interfaces",
            unknown_ct
        ));
        out.push(
            "! Example: security zone VLAN10_USERS / security zone VLAN18_CAMERAS".to_string(),
        );
    }
    out.push(String::new());
    ...
    if unknown_ct > 0 {
        report.add_manual(
            ...
            &format!(
                "{} interfaces require manual zone assignment (see SECTION 1: INFERENCE)",
                unknown_ct
            ),
    ...
    for (iface, z) in cls.iter().filter(|(_, z)| z.zone == ZoneName::Wan) {
        match z.confidence {
            ConfidenceLevel::High => report.add_approximate(
                "security_zone.wan",
...
fn render_change_risk_score(risk: &netconv_core::report::RiskScore, out: &mut Vec<String>) {
    ...
    let score: u32 = risk
        ...
        .iter()
        .map(|r| {
            if r.contains("L2 switch") {
                40
            } else if r.contains("VLANs →") {
                20
            } else if r.contains("STP") {
                15
            } else if r.contains("firewall") {
                10
            } else if r.contains("uncertain WAN") {
                15
            } else {
                5
            }
    ...
    for r in &risk.reasons {
        let pts = if r.contains("L2 switch") {
            40
        } else if r.contains("VLANs →") {
            20
        } else if r.contains("STP") {
...
fn render_migration_complexity(
    cfg: &NetworkConfig,
    ...
    let complexity = if has_vlans && has_stp {
        "HIGH"
    } else if has_l2 || unknown_ct > 10 {
        "MEDIUM"
    } else {
        "LOW"
    };
    ...
    if has_vlans {
        out.push(format!(
            "!     VLAN redesign:          REQUIRED ({} VLANs to re-segment)",
            cfg.vlans.len()
        ));
    }
    if has_stp {
    ...
    if unknown_ct > 0 {
        out.push(format!(
            "!     Zone classification:    REQUIRED ({} interfaces)",
            unknown_ct
        ));
    }
...
fn render_top_risks(cfg: &NetworkConfig, med_wan_ct: usize, out: &mut Vec<String>) {
    ...
    if cfg.stp.is_some() {
        out.push(format!(
            "!   {}. [CRITICAL] STP removal — no L2 loop prevention",
            n
        ));
        out.push("!      BLAST RADIUS: entire L2 domain".to_string());
        out.push(
            "!      SCENARIO: one cable loop → broadcast storm → CPU spike → outage in seconds"
                .to_string(),
        );
        n += 1;
    }
    if !cfg.vlans.is_empty() {
        out.push(format!(
            "!   {}. [HIGH] VLAN collapse — loss of network segmentation",
            n
        ));
        out.push(format!(
            "!      BLAST RADIUS: entire network ({} VLANs merged)",
            cfg.vlans.len()
        ));
    ...
    if med_wan_ct > 0 {
        ...
        out.push(
            "!      SCENARIO: mgmt interface in WAN zone → admin access exposed externally"
                .to_string(),
        );
        n += 1;
    }
    ...
    out.push(
        "!      SCENARIO: bootstrap permit-all applied → unrestricted outbound traffic".to_string(),
    );
    out.push("! ─────────────────────────────────────────────────────────────────────".to_string());
    out.push(String::new());
}

fn render_confidence_by_area(
    cls: &[(&Interface, ZoneClassification)],
    cfg: &NetworkConfig,
    out: &mut Vec<String>,
) {
    let high_ct = cls
        .iter()
        .filter(|(_, z)| z.confidence == ConfidenceLevel::High)
        .count();
    let med_ct = cls
        .iter()
        .filter(|(_, z)| z.confidence == ConfidenceLevel::Medium)
        .count();
    let low_ct = cls
        .iter()
        .filter(|(_, z)| z.confidence == ConfidenceLevel::Low)
        .count();
    let wan_high = cls
        .iter()
        .filter(|(_, z)| z.zone == ZoneName::Wan && z.confidence == ConfidenceLevel::High)
        .count();
    let wan_med = cls
        .iter()
        .filter(|(_, z)| z.zone == ZoneName::Wan && z.confidence == ConfidenceLevel::Medium)
    ...
    out.push(format!(
        "!   Interface roles:     {} high / {} medium / {} unknown",
        high_ct, med_ct, low_ct
    ));
    out.push(format!(
        "!   WAN detection:       {} high, {} medium",
        wan_high, wan_med
    ));
    out.push(format!(
        "!   VLAN reconstruction: {}",
...
fn render_inference_table(
    ...
    if unknown_ct > 0 {
        out.push(format!(
            "! {} interfaces require manual zone assignment:",
            unknown_ct
        ));
        if access_ct > 0 {
            out.push(format!(
                "!   {} access ports → likely LAN endpoints (verify VLAN policy)",
                access_ct
            ));
        }
        if trunk_ct > 0 {
            out.push(format!(
                "!   {} trunk ports  → upstream router=WAN, downstream switch=LAN",
                trunk_ct
            ));
        }
        out.push("!".to_string());
    ...
    for (iface, z) in cls {
        ...
        if z.zone == ZoneName::Wan && z.confidence == ConfidenceLevel::Medium {
            out.push(format!(
                "!           ⚠ RISK: may be management — BLAST RADIUS: exposes admin plane"
            ));
        }
    }
    out.push("! ─────────────────────────────────────────────────────────────────────".to_string());
...
fn render_config_validity_status(
    ...
    let snmp_conflict = cfg
        ...
        .map(|s| {
            s.communities
                .iter()
                .filter(|c| {
                    ["read", "write", "trap", "all"].contains(&c.name.to_lowercase().as_str())
                })
                .count()
        })
        .unwrap_or(0);
    ...
    out.push(format!(
        "!   Status: {}",
        if is_ready {
            "READY (review recommended)"
        } else {
            "NOT READY — do not apply"
        }
    ));
    if !is_ready {
        ...
        if unknown_ct > 0 {
            out.push(format!(
                "!     ✗ {} interfaces have no zone assignment",
                unknown_ct
            ));
        }
        if user_ct > 0 {
        ...
        if med_wan > 0 {
            out.push(format!(
                "!     ✗ {} WAN zone assignments unverified (medium confidence)",
                med_wan
            ));
        }
...
fn render_manual_actions_summary(
    cfg: &NetworkConfig,
    cls: &[(&Interface, ZoneClassification)],
    out: &mut Vec<String>,
) {
    ...
    if unknown_ct > 0 {
        out.push(format!(
            "!   {}. Assign security zones to {} interfaces (see INFERENCE above)",
            step, unknown_ct
        ));
        out.push(
            "!      Then uncomment the '! security-zone ???' lines in interface blocks".to_string(),
        );
        step += 1;
    }
    if user_ct > 0 {
        let names: Vec<_> = cfg
            .users
            .iter()
            .filter(|u| u.name != "enable")
            .map(|u| u.name.as_str())
            .collect();
        out.push(format!(
            "!   {}. Set passwords for: {} (replace <NEW-PASSWORD> placeholders)",
            step,
            names.join(", ")
        ));
        step += 1;
    ...
    if has_enable {
        out.push(format!(
            "!   {}. Configure privileged access (replaces 'enable secret')",
            step
        ));
        step += 1;
    }
    if snmp_conflict > 0 {
        out.push(format!(
            "!   {}. Rename SNMP community — 'write' conflicts with ESR keyword",
            step
        ));
        step += 1;
    }
    if med_wan > 0 {
        out.push(format!(
            "!   {}. Verify WAN interface classification ({} uncertain)",
            step, med_wan
        ));
        step += 1;
    }
    if has_aaa {
        out.push(format!(
            "!   {}. Configure AAA authentication scheme manually",
            step
        ));
        step += 1;
    }
    out.push(format!(
        "!   {}. Replace bootstrap firewall rules with site-specific policy",
        step
    ));
    out.push(String::new());
}

fn render_firewall_safe_mode(
    cfg: &NetworkConfig,
    out: &mut Vec<String>,
    report: &mut ConversionReport,
) {
    ...
    if !cfg.vlans.is_empty() {
        ...
        for vlan in &cfg.vlans {
            if let Some(name) = &vlan.name {
                ...
                let policy =
                    if nl.contains("camera") || nl.contains("security") || nl.contains("cctv") {
...
fn render_architecture_summary(
    ...
    if has_l2 || has_vlans {
        ...
        if trunk_ct + access_ct > 0 {
            out.push(format!(
                "!     Ports: {} trunk uplinks, {} access endpoints",
                trunk_ct, access_ct
            ));
        }
        if has_stp {
            out.push("!     Loop:  STP rapid-pvst active".to_string());
        }
    } else {
...
fn compute_risk_score(
    ...
    if cfg.interfaces.iter().any(|i| i.l2.is_some()) || !cfg.vlans.is_empty() {
        score += 40;
        reasons.push(format!(
            "device class mismatch: L2 switch → L3 router ({} VLANs)",
            cfg.vlans.len()
        ));
    ...
    report.risk = netconv_core::report::RiskScore {
        level,
        reasons,
        recommendations,
    };
